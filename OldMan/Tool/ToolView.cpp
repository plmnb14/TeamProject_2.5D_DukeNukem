...
void CToolView::AddCubeForLoad(CToolTerrain* pTerrain)
{
	CMainFrame* pMainFrm = dynamic_cast<CMainFrame*>(::AfxGetApp()->GetMainWnd());
	NULL_CHECK(pMainFrm);
	...
	pTerrain->SetClicked();
	m_pCubeList.push_back(pTerrain);
	m_mapLayer[ENGINE::CLayer::OBJECT]->AddObject(ENGINE::OBJECT_TYPE::PROPS, pTerrain);
}
...
=======
	m_pSelectCube->SetTexName((wstring)pFormView->m_strObjectName);
	m_mapLayer[ENGINE::CLayer::OBJECT]->AddObject(ENGINE::OBJECT_TYPE::PROPS, m_pSelectCube);
