...
HRESULT CWeapon_SMG::Initialize()
{
	...
	m_tWInfo.fInterval = 0.05f;				// 발사 간격
	m_tWInfo.fKnockBack_Value = 0.1f;		// 저지력

	m_tWInfo.wMaxBullet = 512;				// 최대 탄환 수
	m_tWInfo.wUseBullet = 1;				// 한번 발사 당 소모 탄환 수
	m_tWInfo.wCurBullet = 256;				// 현재  수, 최초 획득 시 탄창 수
	m_tWInfo.wMagazineSize = 40;			// 한 탄창 최대 보관 수
	m_tWInfo.wMagazineBullet = 40;			// 현재 탄창의 총알 개수

	m_tWInfo.fVertical_Rebound = 0.4f;		// 수직 반동
	m_tWInfo.fHorizontal_Rebound = 0.4f;		// 수평 반동

	m_tWInfo.fSpread_X = 50;
	m_tWInfo.fSpread_Y = 50;

	m_tWInfo.fBullet_Speed = 200.f;

	m_tWInfo.wWeaponDamage = 12;				// 무기 데미지
	m_tWInfo.eWeaponTag = ENGINE::SMG;
